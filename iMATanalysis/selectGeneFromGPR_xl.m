        ...
        for j=1:length(curExprArr)
            if length(curExprArr{j})>=1
                ...
                if ~isempty(geneID) %if the gene is measured
                     curExpr= [curExpr, max(gene_exp(geneID))]; %If there is data for any gene in 'AND' rule, take the maximum value
                end
            end
        end
        if ~isempty(curExpr)
        	expressionCol(i)=max(curExpr);%If there is data for any gene in the 'OR' rule, take the maximum value; therefore, all the rxns are turned on regardless of GPR
        end
    end
end